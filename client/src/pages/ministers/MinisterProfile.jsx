...
const MinisterProfile = () => {
  ...
  const handleSubmit = async (e, section) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/minister/minister/${profile._id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      <div className="w-full max-w-4xl bg-white p-8 rounded-lg shadow-lg">
        ...
        {formData && activeSection === 'history' && (
          <div>
            ...
            {Array.isArray(formData.history) && formData.history.length > 0 ? (
              <ul>
                {formData.history.map((item, index) => (
                  <li key={index} className="p-3 border-b border-gray-300">
                    <p><strong>Title:</strong> {item.title}</p>
